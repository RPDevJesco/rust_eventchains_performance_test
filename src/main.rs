    ...
    for (nodes, edges, runs) in test_cases {
        ...
        let stats_traditional = {
            benchmark("Traditional Dijkstra", runs, || {
                let result = dijkstra_traditional(&graph, &source, &target);
        ...
        let stats_bare = {
            benchmark("EventChains (bare - no middleware)", runs, || {
                let result = dijkstra_eventchains_bare(&graph, &source, &target);
                result.distance.is_some()
            })
        };
        ...
        let stats_full = {
            benchmark("EventChains (full middleware)", runs, || {
                let result = dijkstra_eventchains_full(&graph, &source, &target, false);
                result.distance.is_some()
            })
        };
        ...
        let stats_optimized = {
            benchmark("EventChains (optimized)", runs, || {
                let result = dijkstra_eventchains_optimized(&graph, &source, &target);
                result.distance.is_some()
            })
        };
